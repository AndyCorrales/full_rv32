...
typedef struct {
    ...
    XOoo_demo_tick_uio_map maps[ MAX_UIO_MAPS ];
} XOoo_demo_tick_uio_info;

/***************** Variable Definitions **************************************/
static XOoo_demo_tick_uio_info uio_info;
...
static int uio_info_read_name(XOoo_demo_tick_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XOoo_demo_tick_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XOoo_demo_tick_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XOoo_demo_tick_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XOoo_demo_tick_Initialize(XOoo_demo_tick *InstancePtr, const char* InstanceName) {
	XOoo_demo_tick_uio_info *InfoPtr = &uio_info;
